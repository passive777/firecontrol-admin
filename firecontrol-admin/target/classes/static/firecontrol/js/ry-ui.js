    ...
    $.extend({
        ...
        table: {
            ...
            init: function (options) {
                var defaults = {
                    id: options.id == undefined ? "bootstrap-table" : options.id,
                    ...
                    pageSize: 10,
                    pageList: [10, 25, 50],
                    toolbar: options.toolbar == undefined ? "toolbar" : options.toolbar,
                    striped: false,
                    escape: false,
                    showFooter: false,
                    search: false,
            ...
            // 下载设备导入模板
            EquipmentImportTemplate: function () {
                $.get($.table._option.equipmentImportTemplateUrl, function (result) {
                    if (result.code == web_status.SUCCESS) {
                        window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
                    } else if (result.code == web_status.WARNING) {
                        $.modal.alertWarning(result.msg)
            ...
            // 批量导入设备数据
            importExcelAndResult: function (formId) {
        ...
        operate: {
			...
			// 删除设备信息
			removeEquipment: function(id,imei,equipmentName) {
				$.modal.confirm("确定删除该条" + $.table._option.modalName + "信息吗？", function() {
					var url = $.common.isEmpty(id) ? $.table._option.removeUrl : $.table._option.removeUrl.replace("{id}", id);
					if($.table._option.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
						var data = { "ids": id ,"imei":imei,"equipmentName":equipmentName};
						$.operate.submit(url, "post", "json", data);
					}
				});
			},
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.firecontrol_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
